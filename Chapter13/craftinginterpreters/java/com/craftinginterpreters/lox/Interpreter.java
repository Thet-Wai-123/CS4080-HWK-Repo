...
class Interpreter implements Expr.Visitor<Object>,
  ...
  public Object visitInnerExpr(Expr.Inner expr) {
    // Look up the 'inner' callable from the environment
    Object inner = lookUpVariable(expr.keyword, expr);

    // If there's no inner method, return null (no subclass override)
    if (inner == null) {
      return null;
    }

    // 'inner' should be a bound LoxFunction - return it as a callable
    return inner;
  }
