...
//< Methods and Initializers current-class
SwitchCompiler* currentSwitch = NULL;
...
static void continueStatement() {
  if (currentSwitch == NULL) {
    error("Can't use 'continue' outside of a switch.");
  }

  consume(TOKEN_SEMICOLON, "Expect ';' after 'continue'.");

  if (currentSwitch == NULL) return;
  if (currentSwitch->continueJumpCount == UINT8_COUNT) {
    error("Too many continue statements in switch.");
    return;
  }

  currentSwitch->continueJumps[currentSwitch->continueJumpCount++] =
      emitJump(OP_JUMP);
}
...
static void switchStatement() {
  ...
  addLocal(syntheticToken("switch"));
  markInitialized();

  SwitchCompiler switchCompiler;
  switchCompiler.enclosing = currentSwitch;
  switchCompiler.continueJumpCount = 0;
  currentSwitch = &switchCompiler;
  ...
  while (!check(TOKEN_RIGHT_BRACE) && !check(TOKEN_EOF)) {
    if (match(TOKEN_CASE)) {
      ...
      namedVariable(syntheticToken("switch"), false);
      expression();
      ...
      if (seenDefault) {
        error("Can't have more than one default clause.");
      }
      seenDefault = true;
  ...
  for (int i = 0; i < currentSwitch->continueJumpCount; i++) {
    patchJump(currentSwitch->continueJumps[i]);
  }

  currentSwitch = switchCompiler.enclosing;
