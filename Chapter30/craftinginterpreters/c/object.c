...
static ObjString* allocateString(char* chars, int length,
                                 uint32_t hash, bool shortString) {
//< Hash Tables allocate-string
  ObjString* string = ALLOCATE_OBJ(ObjString,
                                   shortString ? OBJ_SHORT_STRING
                                               : OBJ_STRING);
  string->length = length;
  if (shortString) {
    memcpy(string->shortChars, chars, length);
    string->shortChars[length] = '\0';
    string->chars = string->shortChars;
  } else {
    string->chars = chars;
  }
...
//< take-string-intern
  if (length <= SHORT_STRING_MAX_LENGTH) {
    ObjString* string = allocateString(chars, length, hash, true);
    FREE_ARRAY(char, chars, length + 1);
    return string;
  }
...
//< Hash Tables copy-string-hash
  if (length <= SHORT_STRING_MAX_LENGTH) {
    return allocateString((char*)chars, length, hash, true);
  }
