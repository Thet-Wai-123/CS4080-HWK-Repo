...
// Debug-only precedence parser trace. Set to 1 to enable.
...
static const char* parseFnName(ParseFn fn) {
  if (fn == and_) return "and_";
  if (fn == binary) return "binary";
  if (fn == call) return "call";
  if (fn == dot) return "dot";
  if (fn == grouping) return "grouping";
  if (fn == literal) return "literal";
  if (fn == number) return "number";
  if (fn == or_) return "or_";
  if (fn == string) return "string";
  if (fn == super_) return "super_";
  if (fn == this_) return "this_";
  if (fn == unary) return "unary";
  if (fn == variable) return "variable";
  return "<anon>";
}

static void traceIndent() {
  for (int i = 0; i < traceParseDepth; i++) {
    fputs("  ", stderr);
  }
}
#endif
...
#if TRACE_PARSING
  traceIndent();
  fprintf(stderr, "[enter] parsePrecedence(precedence=%s) current=%s('%.*s')\n",
...
//> Global Variables prefix-rule
  ...
  traceIndent();
  fprintf(stderr,
          "[call] %s(canAssign=%d) for prefix token=%s('%.*s')\n",
          parseFnName(prefixRule),
          canAssign,
...
//> infix
  ...
  while (precedence <= getRule(parser.current.type)->precedence) {
    ...
    traceIndent();
    fprintf(stderr,
            "[loop] min=%s, current=%s, currentPrec=%s, nextInfix=%s\n",
            precedenceName(precedence),
            tokenTypeName(parser.current.type),
            precedenceName(currentRule->precedence),
            parseFnName(currentRule->infix));
#endif
    advance();
...
//> Global Variables infix-rule
    ...
    traceIndent();
    fprintf(stderr,
            "[call] %s(canAssign=%d) for infix token=%s('%.*s')\n",
            parseFnName(infixRule),
            canAssign,
            tokenTypeName(parser.previous.type),
            parser.previous.length,
            parser.previous.start);
...
//< Global Variables invalid-assign
  ...
  traceParseDepth--;
  traceIndent();
  fprintf(stderr, "[exit] parsePrecedence(precedence=%s)\n",
          precedenceName(precedence));
#endif
//< infix
//< precedence-body
