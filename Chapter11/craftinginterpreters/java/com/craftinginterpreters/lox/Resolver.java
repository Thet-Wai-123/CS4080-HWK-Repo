...
class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
  ...
  private final Stack<Integer> scopeDepths = new Stack<>();

  private static class Variable {
    final Token name;
    VariableState state;
    final int index; // Index within the scope for array-based access
  ...
  private enum VariableState {
    ...
    DEFINED, // Defined but not yet used
  ...
  public Void visitClassStmt(Stmt.Class stmt) {
    ...
    if (stmt.superclass != null) {
      ...
      scopes.peek().put("super",
          new Variable(new Token(TokenType.IDENTIFIER, "super", null, 0), VariableState.USED, 0));
    }
    // < Inheritance begin-super-scope
    // > resolve-methods
    ...
    beginScope();
    scopes.peek().put("this", new Variable(new Token(TokenType.IDENTIFIER, "this", null, 0), VariableState.USED, 0));
  ...
  private void beginScope() {
    ...
    scopeDepths.push(0); // Start each scope with index 0
  }
  ...
  private void declare(Token name) {
    ...
    // < duplicate-variable
    int index = scopeDepths.peek();
    scope.put(name.lexeme, new Variable(name, VariableState.DECLARED, index));
    scopeDepths.push(scopeDepths.pop() + 1); // Increment index for next variable
  }
  ...
  private void resolveLocal(Expr expr, Token name) {
    for (int i = scopes.size() - 1; i >= 0; i--) {
      ...
      if (scope.containsKey(name.lexeme)) {
        ...
        if (variable != null && variable.state == VariableState.DEFINED) {
