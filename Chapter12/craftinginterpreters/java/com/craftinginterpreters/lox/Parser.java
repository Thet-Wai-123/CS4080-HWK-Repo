...
class Parser {
  ...
  private Stmt classDeclaration() {
    ...
    while (!check(RIGHT_BRACE) && !isAtEnd()) {
      if (match(CLASS)) {
        ...
        // Check if this is a getter (method without parentheses)
        if (check(IDENTIFIER) && checkNext(LEFT_BRACE)) {
          methods.add(getter("method"));
        } else {
  ...
  // Method for parsing getter methods (no parameters)
  private Stmt.Function getter(String kind) {
    Token name = consume(IDENTIFIER, "Expect " + kind + " name.");
    
    // Getters have no parameters, just the body
    consume(LEFT_BRACE, "Expect '{' before " + kind + " body.");
    List<Stmt> body = block();
  ...
  // Helper method to check the token after the current one
