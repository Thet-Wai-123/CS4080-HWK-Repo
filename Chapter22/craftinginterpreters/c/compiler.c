...
//< Local Variables current-compiler
static uint8_t scriptConstGlobals[UINT8_COUNT];
static int scriptConstGlobalCount = 0;
...
//< too-many-upvalues
  ...
  if (isLocal) {
    compiler->upvalues[upvalueCount].isConst =
        compiler->enclosing->locals[index].isConst;
  } else {
    compiler->upvalues[upvalueCount].isConst =
        compiler->enclosing->upvalues[index].isConst;
  }
...
//< Local Variables mark-initialized
static bool isScriptConstGlobal(uint8_t global) {
  for (int i = 0; i < scriptConstGlobalCount; i++) {
    if (scriptConstGlobals[i] == global) return true;
  }
  return false;
}
...
//< Local Variables define-variable
  if (isConst) {
    if (scriptConstGlobalCount == UINT8_COUNT) {
      error("Too many global constants in one chunk.");
    } else {
      scriptConstGlobals[scriptConstGlobalCount++] = global;
    }
  }
...
//> Local Variables named-local
  ...
  if (arg != -1) {
    getOp = OP_GET_LOCAL;
    setOp = OP_SET_LOCAL;
    isConstVar = current->locals[arg].isConst;
//> Closures named-variable-upvalue
  } else if ((arg = resolveUpvalue(current, &name)) != -1) {
    getOp = OP_GET_UPVALUE;
    setOp = OP_SET_UPVALUE;
    isConstVar = current->upvalues[arg].isConst;
//< Closures named-variable-upvalue
  } else {
    arg = identifierConstant(&name);
    getOp = OP_GET_GLOBAL;
    setOp = OP_SET_GLOBAL;
    isConstVar = isScriptConstGlobal((uint8_t)arg);
  }
//< Local Variables named-local
...
//< named-variable-can-assign
    expression();
    if (isConstVar) {
      error("Can't assign to constant variables");
      return;
    }
...
static void constDeclaration() {
  ...
  if (!match(TOKEN_EQUAL)) {
    errorAtCurrent("Expect '=' after constant variable name.");
    emitByte(OP_NIL);
  } else {
    expression();
  }
